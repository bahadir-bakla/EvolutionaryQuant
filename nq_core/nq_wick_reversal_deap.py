"""
NQ Wick Reversal Strategy - DEAP Compatible Version for Genetic Programming
This strategy implements the user's specific request:
- US session first 15-minute range detection (9:30-9:45 EST)
- Wick rejection at liquidation points (range highs/lows)
- Opposite direction entry
- Fixed 20-point profit target
- Designed to work with the existing DEAP optimization framework
"""
...
class NQWickReversalParams:
    """Parameters that can be evolved by DEAP - matches user's exact specifications"""
    # US Session parameters (EXACTLY as requested)
    session_start_hour: int = 9          # 9:30 AM EST
    session_start_minute: int = 30       # 9:30 AM EST
    range_duration_minutes: int = 15     # First 15 minutes EXACTLY as requested
    
    # Wick reversal parameters
    wick_threshold_ratio: float = 0.3    # Minimum wick size as ratio of candle body
    
    # Profit target (EXACTLY as requested)
    profit_target_points: float = 20.0   # 20 units profit target EXACTLY as requested
    
    # Stop loss (user mentioned ATR-based stop)
...
class NQWickReversalStrategy(IStrategy):
    ...
    IMPLEMENTS USER'S EXACT REQUEST:
    - US session first 15-minute range (9:30-9:45 EST)
    - Wick rejection at liquidation points (range highs/lows)
    - Opposite direction entry
    - Fixed 20-point profit target
    """
    ...
    def __init__(self, params: NQWickReversalParams = None):
        ...
        self.name = "NQ_Wick_Reversal_DEAP"
    ...
    def is_us_session_open(self, timestamp) -> bool:
        """Check if we're in the US session opening range (EXACTLY as requested)"""
        hour = timestamp.hour
    ...
    def update_daily_range(self, df: pd.DataFrame, idx: int):
        """Update the daily opening range based on first 15 minutes (EXACTLY as requested)"""
        ...
        if idx < self.params.range_duration_minutes:
            # Still in the opening range period (EXACTLY first 15 minutes)
            ...
            # Just finished establishing the range (EXACTLY after 15 minutes)
            period_data = df.iloc[:idx+1]
            self.daily_range_high = period_data['high'].max()
            self.daily_range_low = period_data['low'].min()
    ...
    def generate_signals(self, data: pd.DataFrame, current_idx: int) -> Signal:
        """Generate trading signal at current index - IMPLEMENTS USER'S EXACT LOGIC"""
        ...
        # Update daily range (EXACTLY first 15 minutes logic)
        self.update_daily_range(self._cached_indicators, current_idx)
        
        # ONLY trade after range is established (EXACTLY as requested)
        if not self.range_established:
            return signal
            
        # ONLY trade during US session after opening range (EXACTLY as requested)
        if not self.is_us_session_open(row.name):
            return signal
        
        # CHECK FOR WICK REJECTIONS AT RANGE LEVELS (USER'S CORE REQUEST)
        return self.check_wick_rejection(row, prev_row)
    
    def check_wick_rejection(self, row, prev_row) -> Signal:
        """Check for wick rejections at range highs/lows - USER'S CORE LOGIC"""
        signal = Signal()
        signal.signal_type = SignalType.NEUTRAL
        ...
        # User's logic: liquidation point at range high + wick rejection = go short
        if (row['high'] > self.daily_range_high and 
            ...
            signal.price = prev_row['close']  # Entry at previous candle's close
            signal.stop_loss = self.daily_range_high + (row['atr'] * self.params.stop_loss_atr_mult)
            signal.take_profit = signal.price - self.params.profit_target_points  # EXACTLY 20 points target
            ...
            signal.metadata = {
                ...
                'wick_type': 'UPPER',
                'range_duration': self.params.range_duration_minutes  # For verification
            }
            
        # CHECK FOR LOWER WICK REJECTION AT RANGE LOW (POTENTIAL LONG)
        # User's logic: liquidation point at range low + wick rejection = go long
